...
public class Game extends  Thread{
	...
	private boolean check_connentivity(int sx, int sy, int tx, int ty){
		if(sx == tx && sy == ty) {
			return true;
		}
		g[sx][sy ] = 1;
		for(int i = 0; i < 4; i++){
			...
			if(x >= 0 && x < this.rows && y >= 0 && y < this.cols && g[x][y] == 0){
				if(check_connentivity(x, y, tx, ty)){
					g[sx][sy] = 0;
					return true;
				}
			}
	...
	private boolean draw(){
		...
		return check_connentivity(this.rows - 2, 1, 1, this.cols - 2);
	}
	...
	private String getInput(Player player){
		...
		return getMapString() + "#" + me.getSx() + "#" + me.getSx() + "#(" +  me.getStepsString() + ")#" +
